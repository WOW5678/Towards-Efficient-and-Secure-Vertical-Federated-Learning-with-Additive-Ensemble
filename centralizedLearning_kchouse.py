...
if __name__ == '__main__':
    ...
    print(config.device)
    # 分别加载三个机构的训练和测试数据集
    trainDataset = dataset.FullKCHouseDataset(config, Flag='train')
    valDataset = dataset.FullKCHouseDataset(config, Flag='val')
    testDataset = dataset.FullKCHouseDataset(config,  Flag='test')
    ...
    bottom_model = models.RegressionBottomModel(in_size=18, hidden_size=200).to(config.device)
    ...
    best_loss = 999999999
    # 将训练过程中的loss写入文件
    with open('centralizedLearning_kchouse_training_loss.csv', 'a+') as csvfile:
        ...
        for epoch in range(config.kchouse_epochs):
            ...
            if loss_val < best_loss:
                ...
                torch.save(bottom_model, os.path.join(config.saved_dir_kchouse, 'centralizedBottomModel.pth'))
                torch.save(top_model, os.path.join(config.saved_dir_kchouse, 'centralizedTopModel.pth'))
    ...
    # 训练结束之后，加载表现最好的模型
    bottom_model = torch.load(os.path.join(config.saved_dir_kchouse, 'centralizedBottomModel.pth'))
    top_model = torch.load(os.path.join(config.saved_dir_kchouse, 'centralizedTopModel.pth'))
