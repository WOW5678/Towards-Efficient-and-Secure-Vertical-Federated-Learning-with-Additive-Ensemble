...
if __name__ == '__main__':
    ...
    print(config.device)
    # 分别加载三个机构的训练和测试数据集
    trainDataset = dataset.KCHouseDataset(config, institution='b', Flag='train')
    valDataset = dataset.KCHouseDataset(config, institution='b', Flag='val')
    testDataset = dataset.KCHouseDataset(config, institution='b', Flag='test')
    ...
    bottom_model = models.RegressionBottomModel(in_size=9, hidden_size=200).to(config.device)
    ...
    # 构建优化器
    optimizer = optim.Adam(list(bottom_model.parameters()) + list(top_model.parameters()), lr=config.learning_rate_kchouse, weight_decay=0.001) # weight_decay用于缓解过拟合现象

    # 模型训练过程
    best_loss = 999999999
    # 将训练过程中的loss写入文件
    with open('onlywithOnePart_kchouse_training_loss.csv', 'a+') as csvfile:
        ...
        for epoch in range(config.kchouse_epochs):
            ...
            if loss_val < best_loss:
                ...
                torch.save(bottom_model, os.path.join(config.saved_dir_kchouse, 'onePartBottomModel.pth'))
                torch.save(top_model, os.path.join(config.saved_dir_kchouse, 'onePartTopModel.pth'))
    ...
    start_time = time.time()
    # 训练结束之后，加载表现最好的模型
    bottom_model = torch.load(os.path.join(config.saved_dir_kchouse, 'onePartBottomModel.pth'))
    top_model = torch.load(os.path.join(config.saved_dir_kchouse, 'onePartTopModel.pth'))
