...
if __name__ == '__main__':
    ...
    print(config.device)
    # 分别加载三个机构的训练和测试数据集
    trainDataset = dataset.FullCifar10Dataset(config, Flag='train')
    valDataset = dataset.FullCifar10Dataset(config, Flag='val')
    testDataset = dataset.FullCifar10Dataset(config,  Flag='test')
    ...
    bottom_model = models.Cifar10_BottomModel(in_channels=3, out_channels=6, kernel_size=5, hidden_size=200).to(config.device)
    ...
    # 构建优化器
    optimizer = optim.Adam(list(bottom_model.parameters()) + list(top_model.parameters()), lr=config.learning_rate_cifar10, weight_decay=0.001) # weight_decay用于缓解过拟合现象

    # 模型训练过程
    best_acc = 0.0

    # 将训练过程中的loss写入文件
    with open('centralizedLearning_cifar10_training_loss_2.csv', 'a+') as csvfile:
        ...
        for epoch in range(config.cifar10_epochs):
            ...
            if acc_val > best_acc:
                ...
                torch.save(bottom_model, os.path.join(config.saved_dir_cifar10, 'centralizedBottomModel.pth'))
                torch.save(top_model, os.path.join(config.saved_dir_cifar10, 'centralizedTopModel.pth'))
    ...
    start_time = time.time()
    # 训练结束之后，加载表现最好的模型
    bottom_model = torch.load(os.path.join(config.saved_dir_cifar10, 'centralizedBottomModel.pth'))
    top_model = torch.load(os.path.join(config.saved_dir_cifar10, 'centralizedTopModel.pth'))
