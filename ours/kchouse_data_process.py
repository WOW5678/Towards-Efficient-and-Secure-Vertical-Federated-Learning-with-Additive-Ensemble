...
def featureNormalization(data):
	...
	x_minmax.fit(data)
	x_scaled = x_minmax.transform(data)  # 归一化x
	return x_scaled

def load_kchouse_price_data(data_root):
...
if __name__ == '__main__':
    # 加载数据集
	root = '/home/nbic/wangshanshan2023/additiveVFL/data/kc-house-price'
	train_data, val_data, test_data = load_kchouse_price_data(os.path.join(root, 'kc_house_data.csv'))

	# 将数据集保存下来
	np.savez(os.path.join(root, "kchouse.train.npz"), train_data)
	np.savez(os.path.join(root, "kchouse.val.npz"), val_data)
	...
	np.savez(os.path.join(root, "kchouse.test.npz"), test_data)
