...
if __name__ == '__main__':
    ...
    print(config.device)
    # 分别加载三个机构的训练和测试数据集
    trainDataset = dataset.AdultDataset(config, institution='b', Flag='train')
    valDataset = dataset.AdultDataset(config, institution='b', Flag='val')
    testDataset = dataset.AdultDataset(config, institution='b', Flag='test')
    ...
    model = models.Centralized(in_size=57, hidden_size=200).to(config.device)
    ...
    best_acc = 0.0
    # 将训练过程中的loss写入文件
    with open('onlywithOnePart_adult_training_loss_3.csv', 'w') as csvfile:
        ...
        for epoch in range(config.adult_epochs):
            ...
            if acc_val > best_acc:
                best_acc = acc_val
                # 保存模型
                torch.save(model, os.path.join(config.saved_dir_adult, 'onlywithOnePart.pth'))
    ...
    start_time = time.time()
    # 训练结束之后，加载表现最好的模型
    model = torch.load(os.path.join(config.saved_dir_adult, 'onlywithOnePart.pth'))
