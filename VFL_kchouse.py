...
if __name__ == '__main__':
    ...
    bottom_model_a = models.BottomModel(in_size=9, hidden_size=200).to(config.device)
    bottom_model_b = models.BottomModel(in_size=9, hidden_size=200).to(config.device)
    ...
    optimizer_top = optim.Adam(top_model.parameters(), lr=config.learning_rate_kchouse)


    # 模型训练过程
    best_mse = 99999999
    # 将训练过程中的loss写入文件
    with open('VFL_kchouse_training_loss.csv', 'a+') as csvfile:
        ...
        for epoch in range(config.kchouse_epochs):
            ...
            if loss_val < best_mse:
                ...
                torch.save(bottom_model_a, os.path.join(config.saved_dir_kchouse, 'bottom_model_a_VFL_2parts.pth'))
                torch.save(bottom_model_b, os.path.join(config.saved_dir_kchouse, 'bottom_model_b_VFL_2parts.pth'))
                torch.save(top_model, os.path.join(config.saved_dir_kchouse, 'top_model_VFL_2parts.pth'))
    ...
    # 训练结束之后，加载表现最好的模型
    bottom_model_a = torch.load(os.path.join(config.saved_dir_kchouse, 'bottom_model_a_VFL_2parts.pth'))
    bottom_model_b = torch.load(os.path.join(config.saved_dir_kchouse, 'bottom_model_b_VFL_2parts.pth'))
    top_model = torch.load(os.path.join(config.saved_dir_kchouse, 'top_model_VFL_2parts.pth'))
